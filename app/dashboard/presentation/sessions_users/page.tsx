...
import { ArrowUpDown, DollarSign, Car, Clock, CheckCircle } from "lucide-react";
...
export default function SessionsUsersPage() {
    ...
    const fetchSessions = async () => {
      ...
      try {
        ...
        const response = await fetch(
          "https://smartpark.htl-projekt.com/api_getSessionsU.php",
          {
            method: "POST",
            headers: {
  ...
  const getFilteredAndSortedSessions = () => {
    ...
    if (sortConfig.key) {
      filtered.sort((a, b) => {
        const aValue = a[sortConfig.key as keyof Session];
        const bValue = b[sortConfig.key as keyof Session];
  ...
  const stats = {
    ...
    totalEarnings: sessions.reduce(
      (sum, s) => sum + (typeof s.cost === "number" && !Number.isNaN(s.cost) ? s.cost : 0),
      0
    ),
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b bg-gray-50/50">
                      ...
                      <th className="text-left py-4 px-3 font-semibold text-gray-700">
                        <button
                          onClick={() => handleSort("parkPlaceID")}
                          className="flex items-center gap-2 hover:text-gray-900"
                        >
                          Parking Spot
                          <ArrowUpDown className="w-4 h-4" />
